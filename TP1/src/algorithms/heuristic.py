...
def heuristic_1(current_color, node_new_color, grid):
    ...
    waiting_list = []           # Lista de espera, del current color que faltan por analizar 
...
def non_visited_zone(current_coord, current_color, grid, visited, waiting_list):
    ...
    for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]: # Arriba, derecha, abajo, izquierda
            x, y = current_x + dx, current_y + dy
            if(0 <= x < len(grid[0]) and 0 <= y < len(grid) and grid[x][y] == current_color and ((x,y) not in waiting_list) and ((x,y) not in visited) ): # Ver que este en grilla
...
cuenta el caso de querer pasar al color en el que ya estaba previamente ) y devuelvo ese 1/puntaje
...
Ejemplo:
   ...
   Devuelto sera 1/3, ya que hay 3 celdas verdes sin tomar
        y
   x  |  rojo | verde     | rojo   |
      |  azul |  rojo     | verde   |  
      | verde | amarillo  | blanco |
